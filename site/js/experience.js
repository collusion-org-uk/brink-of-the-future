...
// JG update 23/8/17 to solve  rejection of modal bug
function promptPlayBack(zone){
	...
	$('#audio-prompt').unbind('click').click(function(){
		zone.zoneSound.play();
		// remove the hide listener before hiding otherwise interpreted as no sound selected
		removeHideListener();
		audioModal.modal('hide');	
	});
	// remove listenr if exists before adding new
	removeHideListener();
	// listen to the modal being hidden, 
	 audioModal.on('hide.bs.modal', function () {
	    // assume the sound modal was rejected so reinstate associated vars to reflect no sound selected
		previousNarration = null;
		narrationPlaying = false;	
	});
}
function removeHideListener(){
	// stop listening to the event that is triggered when the modal is closed 
	var audioModal = $('#audioZoneModal');
	audioModal.unbind('hide.bs.modal');
}
// geometric functions
